...
export const demoCases: DemoCase[] = [
  { case_id: "checkout-failure", title: "Checkout failure", description: "Authored checkout example with signals for a bounded local run.", service: "checkoutservice", telemetry_origin: "controlled_runtime", fixture_kind: "authored_synthetic_controlled_fixture", source_interval: { start: "2026-09-13T08:00:00Z", end: "2026-09-13T08:10:00Z" } },
  { case_id: "degraded-performance", title: "Degraded performance", description: "Authored latency example where a nearby release clue is not causal proof.", service: "checkoutservice", telemetry_origin: "controlled_runtime", fixture_kind: "authored_synthetic_controlled_fixture", source_interval: { start: "2026-09-13T09:00:00Z", end: "2026-09-13T09:10:00Z" } },
...
const STORAGE_KEY = "incident-lens:public-demo:v1";
...
type DemoEvidence = { evidence_id: string; source_type: "log" | "metric" | "trace" | "runbook"; source: typeof DEMO_SOURCE; event_time: string; query_window: DemoRun["provenance"]["source_interval"]; content_or_summary: string; quality_flags: string[]; access_scope: "guest_session" | "public_knowledge" };
...
function executeWorkflow(run: DemoRun): void {
  ...
  const evidence: DemoEvidence[] = (["log", "metric", "trace"] as const).map((signal) => ({
    evidence_id: `demo-evidence-${run.run_id}-${signal}`,
    source_type: signal,
    source: DEMO_SOURCE,
    event_time: missing.includes(signal) ? interval.start : interval.start,
    query_window: interval,
    content_or_summary: missing.includes(signal) ? `No ${signal} records returned for the selected authored interval.` : `Authored fixture returned bounded ${signal} records for the selected interval.`,
    quality_flags: missing.includes(signal) ? ["missing"] : [],
    access_scope: "guest_session",
  }));
  if (!run.withheld) evidence.push({ evidence_id: `demo-evidence-${run.run_id}-runbook`, source_type: "runbook", source: DEMO_SOURCE, event_time: interval.start, query_window: interval, content_or_summary: "Authored fixture guidance: compare the adjacent baseline before making a causal claim.", quality_flags: [], access_scope: "public_knowledge" });
  ...
  const finding: DemoFinding = {
    ...
    run_id: run.run_id,
    assessment: missing.length ? `Evidence is incomplete: ${missing.join(", ")} signal(s) are missing. No causal conclusion is supported.` : run.case_id === "degraded-performance" ? "The authored fixture supports an unusual service/window ranking (latency sample 640ms); a nearby release clue would not prove causality." : "The authored fixture returned 4 bounded records, including 1 error record; this supports an unusual service/window ranking but does not prove causality.",
