...
export default function UploadPanel({ sessionId, onState }: UploadPanelProps) {
  ...
  const upload = async (file: File) => {
    ...
    setBusy(true);
    try {
      const initiatedResponse = await fetch("/v1/uploads/initiate", { method: "POST", headers: { "X-Session-ID": sessionId } });
      ...
      const response = await fetch(`/v1/uploads/${initiated.upload_id}`, {
        method: "PUT",
  ...
  return <section className="upload-panel" aria-labelledby="upload-heading">
    ...
    <div className="upload-actions"><a className="upload-sample" href="/v1/uploads/sample" download>Download sample</a><label className="upload-input">Choose JSONL<input type="file" accept=".jsonl,application/jsonl" disabled={!sessionId || busy} onChange={(event) => { const file = event.target.files?.[0]; if (file) void upload(file); event.currentTarget.value = ""; }} /></label></div>
